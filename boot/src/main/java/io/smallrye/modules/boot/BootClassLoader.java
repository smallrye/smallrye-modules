...
public final class BootClassLoader extends ClassLoader {
    ...
    protected Class<?> loadClass(String name, boolean resolve) throws ClassNotFoundException {
        if (name.startsWith("[")) {
            return loadClassByDescriptor(name, false);
        }
        ...
        Class<?> clazz = loadClassDirect(binaryName);
        if (resolve) {
            resolveClass(clazz);
        }
        return clazz;
    ...
    private Class<?> loadClassByDescriptor(String name, boolean resolve) throws ClassNotFoundException {
        return switch (name.charAt(0)) {
            ...
            case '[' -> loadClassByDescriptor(name.substring(1), resolve).arrayType();
            case 'L' -> loadClass(name.substring(0, name.length() - 1), resolve);
